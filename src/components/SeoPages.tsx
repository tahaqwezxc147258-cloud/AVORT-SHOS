...
export const SeoPages: React.FC<{ path:string }> = ({path}) => {
  ...
  return <main className="max-w-4xl mx-auto px-4 py-12 min-h-[60vh]" dir="rtl">{article ? <><p className="text-cyan-600 font-bold mb-3">مجله Avort</p><h1 className="text-3xl font-black text-slate-900">{article.title}</h1><p className="mt-5 text-lg leading-9 text-slate-600">{article.intro}</p>{article.sections.map(s=><section key={s.heading} className="mt-8"><h2 className="text-xl font-black text-slate-900">{s.heading}</h2><p className="mt-3 leading-9 text-slate-600">{s.body}</p></section>)}</> : <><h1 className="text-3xl font-black text-slate-900">{page?.title || 'مقالات راهنمای خرید کفش'}</h1><p className="mt-6 leading-9 text-slate-600">{page?.body || 'راهنماهای کاربردی انتخاب، خرید و نگهداری کفش را در مجله Avort بخوانید.'}</p></>}</main>;
};
