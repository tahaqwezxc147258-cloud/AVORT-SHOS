...
export const SeoHead: React.FC = () => {
  const { viewMode, products } = useStore();
  useEffect(() => {
    ...
    link.href = canonical;
    const data = article ? { '@context': 'https://schema.org', '@type': 'Article', headline: article.title, description, url: canonical, author: { '@type': 'Organization', name: 'آورت' }, publisher: { '@type': 'Organization', name: 'آورت' }, inLanguage: 'fa-IR' } : { '@context': 'https://schema.org', '@type': ['Organization', 'Store'], name: 'آورت', url: siteUrl, description, areaServed: 'IR', inLanguage: 'fa-IR' };
